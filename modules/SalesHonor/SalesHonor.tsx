import React, { useState, lazy, Suspense, memo } from 'react';
import Spinner from '../../shared/ui/Spinner';

const SalesHonorV1 = lazy(() => import('./v1/SalesHonorV1'));
const SalesHonorV2 = lazy(() => import('./v2/SalesHonorV2'));

const LayoutButton: React.FC<{
  label: string;
  isActive: boolean;
  onClick: () => void;
}> = memo(({ label, isActive, onClick }) => (
  <button
    onClick={onClick}
    className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors ${
      isActive
        ? 'bg-blue-600 text-white shadow-sm'
        : 'text-slate-600 hover:bg-slate-200'
    }`}
  >
    {label}
  </button>
));


const SalesHonorPage: React.FC = () => {
    const [layout, setLayout] = useState('v2');

    const SuspenseFallback: React.FC = () => (
        <div className="flex w-full items-center justify-center p-8">
          <Spinner />
        </div>
      );

    return (
        <div>
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-slate-800">Sales Honor Roll</h1>
                <p className="mt-2 text-slate-600">
                  Recognizing the outstanding achievements of our top sales professionals this month.
                </p>
            </div>

            <div className="mb-6 flex items-center flex-wrap gap-2 border-b border-slate-200 pb-4">
                <span className="text-sm font-medium text-slate-600 mr-2">View Layout:</span>
                <LayoutButton label="Version 1" isActive={layout === 'v1'} onClick={() => setLayout('v1')} />
                <LayoutButton label="Version 2: Podium ⭐" isActive={layout === 'v2'} onClick={() => setLayout('v2')} />
            </div>

            <Suspense fallback={<SuspenseFallback />}>
                {layout === 'v1' && <SalesHonorV1 />}
                {layout === 'v2' && <SalesHonorV2 />}
            </Suspense>
        </div>
    );
};

export default SalesHonorPage;