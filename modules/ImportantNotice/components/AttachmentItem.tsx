...
export const AttachmentItem: React.FC<{ attachment: NoticeAttachment }> = ({ attachment }) => {
  return (
    <a
      href="#"
      onClick={(e) => e.preventDefault()}
      className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg hover:bg-slate-100 border border-slate-200 transition-colors group"
    >
      <div className="flex-shrink-0 h-10 w-10 flex items-center justify-center bg-white rounded-md shadow-sm">
         <FileIcon type={attachment.type} />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-slate-800 truncate group-hover:text-blue-600">{attachment.name}</p>
